...
Celem laboratorium było zaimplementowanie oraz porównanie algorytmów Dijkstry, A* oraz Bellmana-Forda w problemie wyznaczania najkrótszej ścieżki w grafie dwuwymiarowym reprezentującym siatkę.
...
Algorytm Dijkstry jest klasycznym rozwiązaniem problemu najkrótszej ścieżki, które gwarantuje znalezienie optymalnej trasy w grafach z nieujemnymi wagami krawędzi. Algorytm A* stanowi rozszerzenie metody Dijkstry poprzez wprowadzenie funkcji heurystycznej, która kieruje przeszukiwaniem w stronę celu, co często prowadzi do szybszego znalezienia rozwiązania. Algorytm Bellmana-Forda, choć znacznie wolniejszy, ma tę przewagę, że potrafi obsłużyć grafy z ujemnymi wagami krawędzi oraz wykrywać ujemne cykle.

W pracy przeprowadzono pomiary czasów wykonania wszystkich trzech algorytmów w zależności od rozmiaru siatki. Ze względu na znacznie wyższą złożoność obliczeniową algorytmu Bellmana-Forda, jego pomiary ograniczono do małych instancji problemu (siatki do $50 \times 50$ wierzchołków). Uzyskane wyniki umożliwiają ocenę wydajności badanych metod oraz porównanie efektywności różnych podejść do rozwiązywania problemu najkrótszej ścieżki.
...
W efekcie, choć algorytmy Dijkstry i A* mają podobną asymptotyczną złożoność, A* jest szybszy w praktyce dzięki zmniejszeniu liczby odwiedzanych wierzchołków. Dla badanej siatki $1000 \times 1000$ A* był o około 30\% szybszy od Dijkstry. Z kolei algorytm Bellmana-Forda, mimo swojej uniwersalności (obsługa ujemnych wag), jest o kilka rzędów wielkości wolniejszy dla większych instancji.
...
Algorytm Bellmana-Forda jest klasycznym algorytmem służącym do znajdowania najkrótszych ścieżek od jednego źródła do wszystkich pozostałych wierzchołków w grafie. W przeciwieństwie do algorytmu Dijkstry, Bellman-Ford może obsługiwać grafy z ujemnymi wagami krawędzi i dodatkowo wykrywać obecność ujemnych cykli.
...
Złożoność czasowa algorytmu wynosi $O(V \cdot E)$, gdzie $V$ to liczba wierzchołków, a $E$ to liczba krawędzi. Dla siatki dwuwymiarowej $N \times N$ mamy $V = N^2$ i $E \approx 4N^2$, co daje złożoność $O(N^4)$ — znacznie gorszą niż $O(N^2 \log N)$ dla Dijkstry i A*.

W implementacji testowej algorytm Bellmana-Forda był używany tylko dla małych instancji ze względu na jego niską wydajność. Jednak w zastosowaniach wymagających obsługi ujemnych wag lub wykrywania ujemnych cykli algorytm ten pozostaje niezbędnym narzędziem.
...
W ramach przeprowadzonych badań przeanalizowano trzy algorytmy wyznaczania najkrótszej ścieżki w grafie dwuwymiarowym: algorytm Dijkstry, algorytm A* oraz algorytm Bellmana-Forda. Otrzymane wyniki pozwoliły na wszechstronną ocenę wydajności różnych podejść do rozwiązywania tego fundamentalnego problemu teorii grafów.

Algorytm Dijkstry, będący klasycznym rozwiązaniem problemu najkrótszej ścieżki, charakteryzuje się prostotą implementacji i gwarantuje znalezienie optymalnej trasy w grafach z nieujemnymi wagami. Jest to solidny wybór, gdy nie dysponujemy dobrą funkcją heurystyczną lub gdy priorytetem jest prostota kodu. Jego złożoność czasowa wynosi $O(N^2 \log N)$ dla siatki $N \times N$.
...
Algorytm Bellmana-Forda, choć o znacznie wyższej złożoności czasowej $O(N^4)$ dla siatki dwuwymiarowej, ma unikalne właściwości — potrafi obsługiwać grafy z ujemnymi wagami krawędzi oraz wykrywać ujemne cykle. W pomiarach dla małych instancji (do $50 \times 50$) okazał się on być ponad 1000 razy wolniejszy od Dijkstry i A* dla większych siatek. Ze względu na tę znaczną różnicę w wydajności algorytm Bellmana-Forda jest praktyczny tylko dla małych grafów lub w sytuacjach wymagających obsługi ujemnych wag.

Pomiary czasowe wykazały, że:
\begin{itemize}
    \item Dla małych instancji (do $300 \times 300$) różnica w wydajności między Dijkstrą a A* jest niewielka, jednak staje się ona znacząca wraz ze wzrostem rozmiaru problemu.
    \item Algorytm A* odwiedza mniej wierzchołków w praktyce dzięki heurystyce, co przekłada się na lepszą wydajność.
    \item Algorytm Bellmana-Forda, mimo swojej uniwersalności, jest niepraktyczny dla większych instancji ze względu na bardzo wysoką złożoność obliczeniową.
\end{itemize}
...
\begin{itemize}
    ...
    \item gdy graf może zawierać ujemne wagi krawędzi lub wymagane jest wykrywanie ujemnych cykli — algorytm Bellmana-Forda jest jedyną opcją spośród badanych, jednak należy liczyć się z bardzo długim czasem wykonania dla większych instancji.
\end{itemize}
